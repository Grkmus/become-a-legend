...
export default {
  ...
  methods: {
    fetchEvent: async function(id) {
      this.loading = true
      const res = await axios.get(`http://backend.docker.localhost/daily-event/${id}`)
      this.event = res.data
      this.loading = false
    },
    fetchEventPhase2: async function(id) {
      this.loading = true
      const res = await axios.post(`http://backend.docker.localhost/daily-event/phase2/`, {_id: this.event._id})
      this.event = res.data
    ...
    handleSkipTurn: async function() {
      this.event.turn++
      await axios.put(`http://backend.docker.localhost/daily-event/${this.event._id}`, this.event )
    },
    handleSelection: async function(id){
      ...
      await axios.put(`http://backend.docker.localhost/team/${selectedTeam._id}`, selectedTeam)
      this.teamQuota--
      if(this.teamQuota == 0) {
